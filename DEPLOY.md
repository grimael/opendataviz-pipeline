...
- **Données figées entre deux déploiements** : comme il n'y a pas de disque persistant, les données ne se rafraîchissent qu'au *build* de l'image. C'est géré automatiquement : `.github/workflows/etl.yml` tourne chaque jour à 6h UTC, régénère les données, commit le JSON du dashboard (Render redéploie le site statique automatiquement sur ce push) puis appelle le *Deploy Hook* de Render pour reconstruire l'image de l'API avec les données fraîches — les deux services restent synchronisés sur le même cycle quotidien, sans action manuelle. L'URL du hook est stockée comme secret GitHub (`RENDER_DEPLOY_HOOK_URL`, Settings → Secrets and variables → Actions du dépôt), jamais dans le code. Pour rafraîchir manuellement entre deux exécutions du cron : onglet **Actions** du dépôt GitHub → workflow **Daily ETL Pipeline** → **Run workflow**.
